...
class MessageStore:
    ...
    def delete_message(self, message_id: bytes) -> bool:
        """
    ...
    def clear_inbox(self, recipient_id: bytes) -> int:
        """
        Delete all delivered messages for a recipient.
        ...
        with self._lock:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "DELETE FROM messages WHERE recipient_id = ? AND status = ?",
                    (recipient_id, MessageStatus.DELIVERED),
                )
                return cursor.rowcount
