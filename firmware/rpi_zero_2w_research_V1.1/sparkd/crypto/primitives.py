...
def blake2b_hash(
    ...
    # Use hashlib.blake2b which natively supports variable digest_size,
    # keyed hashing, and personalization (unlike some versions of the
    # cryptography library's BLAKE2b which only allows digest_size=64)
    kwargs = {"digest_size": digest_size}
    if key is not None:
        kwargs["key"] = key
    if person is not None:
        kwargs["person"] = person.ljust(16, b'\x00')
    
    hasher = hashlib.blake2b(**kwargs)
    hasher.update(data)
