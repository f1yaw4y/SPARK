...
def verify_node_id_binding(node_id: bytes, ed25519_public_bytes: bytes) -> bool:
    """
    Verify that a node_id was correctly derived from an Ed25519 public key.
    
    This ensures that a claimed (node_id, public_key) pair is
    mathematically consistent -- the node_id really is the BLAKE2b
    hash of the supplied Ed25519 key.  Used to prevent impersonation
    in beacons and gossip.
    
    Args:
        node_id: Claimed 16-byte node ID
        ed25519_public_bytes: 32-byte Ed25519 public key
        
    Returns:
        bool: True if node_id == BLAKE2b(ed25519_key, person="spark-nodeid")[:16]
    """
