...
DELIVERY_HEADER_SIZE = (
    ...
    16 +  # dest_node_id
    16 +  # message_id
    16 +  # receipt_token (authenticated E2E ACK)
    8 +   # timestamp
    1 +   # payload_type
    2     # payload_length
)  # = 60 bytes
...
class DeliveryInfo:
    ...
    The receipt_token (16 bytes) is a random value embedded by the
    sender.  Only the true recipient can decrypt this layer and
    extract the token.  The recipient returns it in the E2E ACK,
    proving that the message was actually delivered (prevents
    malicious nodes from forging delivery confirmations).
    """
    ...
    def from_bytes(cls, data: bytes) -> 'DeliveryInfo':
        ...
        (layer_type, dest_node, msg_id, receipt_tok, timestamp,
...
def build_onion(
    ...
    # TTL
    ttl: int = 64,
    # Authenticated E2E ACK token
    receipt_token: bytes = b"",
    ...
    Args:
        ...
        receipt_token: 16-byte token for authenticated E2E ACK.
            If empty, a random token is generated automatically.
        
    Returns:
        Tuple of (OnionPacket, receipt_token):
            OnionPacket: Complete onion packet ready for transmission
            receipt_token: The 16-byte token embedded in the delivery layer
        
    Raises:
        OnionError: If construction fails
    """
    # Generate receipt token if not provided
    if not receipt_token:
        receipt_token = os.urandom(16)
    if len(receipt_token) != 16:
        raise OnionError(f"Invalid receipt_token length: {len(receipt_token)}")
    ...
    return (OnionPacket(outer_envelope=layer1_envelope), receipt_token)
...
def build_onion_1layer(
    ...
    return (OnionPacket(outer_envelope=layer_envelope), receipt_token)
...
def max_onion_payload(num_layers: int, max_packet_payload: int = 245) -> int:
    ...
    if num_layers == 3:
        overhead = (
            ...
            DELIVERY_HEADER_SIZE        # 60 (includes 16-byte receipt token)
        )  # = 248
    elif num_layers == 2:
        overhead = (
            2 * ENVELOPE_OVERHEAD +     # 2 x 48 = 96
            1 * ROUTING_HEADER_SIZE +   # 1 x 22 = 22
            DELIVERY_HEADER_SIZE        # 60
        )  # = 178
    elif num_layers == 1:
        overhead = (
