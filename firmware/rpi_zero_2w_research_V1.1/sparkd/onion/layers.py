...
class OnionBuilder:
    ...
    def build_message(
        ...
        payload_type: int = 0,
        ttl: int = 64,
    ) -> Tuple[OnionPacket, bytes, int, bytes]:
        """
        ...
        Returns:
            Tuple of (OnionPacket, message_id, num_layers, receipt_token)
        ...
        if len(msg_data) <= MAX_PAYLOAD_3LAYER:
            ...
            return packet, message_id, 3, token
        
        # Try 2-layer (one relay hop)
        if len(msg_data) <= MAX_PAYLOAD_2LAYER:
            packet, token = build_onion_2layer(
                dest_node_id=recipient_id,
                message_id=message_id,
                timestamp=timestamp,
                payload_type=msg_type,
        ...
        if len(msg_data) <= MAX_PAYLOAD_1LAYER:
            ...
            return packet, message_id, 1, token
